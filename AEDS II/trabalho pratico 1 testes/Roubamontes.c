...
// verifica se a carta no topo da pilha tem o mesmo numero da carta da vez
...
int main(void) {
  ...
  while (1) {// loop base do jogo, sai quando o monte de compras esta vazio
    ...
    while (1) {//loop do jogador, sai se colocar carta no descarte
      ...
      for (int i = 0; i < qtdJ; i++) {
        if(indice == i){
          printf("%d - Colocar no sua pilha de pontos\n", i+1);
        }else{
          printf("%d - Roubar pilha de pontos de %s\n", i+1, jogador[i].nome);
        }
      }
      ...
      while(1){// loop de jogada, sai se jogada for possivel 
        ...
        while (1) { // loop de escolha, sai se escolha for valida
          ...
          scanf("%d", &escolha);
          getchar();
          if (escolha >= 0 && escolha <= qtdJ) {
            break;
          } else {
            printf("Opcao invalida\n");
          }
        }
        if (escolha == 0) { // escolhe jogar no descarte
          ...
          break;
        }else if(escolha >= 1){//pretende roubar de alguem
          escolha--;
          if(indice == escolha && podeRoubar(cartaDaVez, &jogador[indice].roubo)){
            enPilha(&jogador[indice].roubo, cartaDaVez);
            break;
          }else if(podeRoubar(cartaDaVez, &jogador[escolha].roubo)){
            for (int i=0; i<jogador[escolha].roubo.topo; i++){
              enPilha(&jogador[indice].roubo, jogador[escolha].roubo.vetor[i]);
            }
            enPilha(&jogador[indice].roubo, cartaDaVez);
            jogador[escolha].roubo.topo = 0;
            break;
          }else{
